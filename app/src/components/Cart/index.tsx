...
export function Cart({ cartItems, onAdd, onRemove, onConfirmOrder, selectedTable }: CartProps) {
  ...
  const [isConfirmedOrderModalVisible, setIsConfirmedOrderModalVisible] = useState(false);

  const total = cartItems.reduce(
    (acc, cartItem) => acc + cartItem.quantity * cartItem.product.price, 0
  );
          ...
          renderItem={({ item: cartItem }) => (
            <Item>
              <ProductContainer>
                <Image
                  source={{
                    uri: `http://192.168.15.48:3001/uploads/${cartItem.product.imagePath}`,
                  }}
                />
                <QuantityContainer>
